...
// the data this app stores about a customer. We only key Commission rows
// by name/email (no separate Customer table), so log for manual export
// today; automate a lookup-by-email export before public launch.
export const action = async ({ request }) => {
  const { shop, topic, payload } = await authenticate.webhook(request);
  console.log(`Received ${topic} webhook for ${shop}`, payload);
  const email = payload?.customer?.email;
  if (email) {
    const resources = await db.commission.findMany({ where: { merchant: { shop }, customerEmail: email }, select: { id: true } });
